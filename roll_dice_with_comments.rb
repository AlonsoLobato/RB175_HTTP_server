...
# helper method created to parse the URL into different parts we need to interact with the server
def parse_request(request_line)
  ...
  [http_method, path, params] # return values from the method is an array with the three main parts of the URL (request)
end
...
loop do # main Ruby code in the application
  ...
  next if !request_line || request_line =~ /favicon/ # This will terminate the loop early if the request generates a problem, allowing us to know that the rest of our code will not need to handle these situations.
  ...
  # client.puts request_line # this displays the request line to the browsers

  http_method, path, params = parse_request(request_line) # Executing the helper method to parse the URL
